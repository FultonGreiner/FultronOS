...
void XEmacPs_ResetHw(uint32_t BaseAddr)
{
	...
	RegVal &= ~((uint32_t)XEMACPS_NWCTRL_TXEN_MASK|
				(uint32_t)XEMACPS_NWCTRL_RXEN_MASK|
				(uint32_t)XEMACPS_NWCTRL_HALTTX_MASK|
	...
	XEmacPs_WriteReg(BaseAddr,XEMACPS_TXSR_OFFSET,(XEMACPS_TXSR_ERROR_MASK|
									(uint32_t)XEMACPS_TXSR_TXCOMPL_MASK|
									(uint32_t)XEMACPS_TXSR_TXGO_MASK));
	/* Clear the rx status */
