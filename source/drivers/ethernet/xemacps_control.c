 ...
 *
 *****************************************************************************/
long XEmacPs_SetMacAddress(XEmacPs *InstancePtr, void *AddressPtr, uint8_t Index)
{
	uint32_t MacAddr;
	...
	if (InstancePtr->IsStarted == (uint32_t)XIL_COMPONENT_IS_STARTED) {
	...
	XEmacPs_WriteReg(InstancePtr->Config.BaseAddress,
				((uint32_t)XEMACPS_LADDR1L_OFFSET + ((uint32_t)IndexLoc * (uint32_t)8)), MacAddr);

	/* There are reserved bits in TOP so don't affect them */
	MacAddr = XEmacPs_ReadReg(InstancePtr->Config.BaseAddress,
					((uint32_t)XEMACPS_LADDR1H_OFFSET + ((uint32_t)IndexLoc * (uint32_t)8)));

		MacAddr &= (uint32_t)(~XEMACPS_LADDR_MACH_MASK);
	...
	XEmacPs_WriteReg(InstancePtr->Config.BaseAddress,
				((uint32_t)XEMACPS_LADDR1H_OFFSET + ((uint32_t)IndexLoc * (uint32_t)8)), MacAddr);
 ...
 *
 *****************************************************************************/
void XEmacPs_GetMacAddress(XEmacPs *InstancePtr, void *AddressPtr, uint8_t Index)
{
	uint32_t MacAddr;
	uint8_t *Aptr = (uint8_t *)(void *)AddressPtr;
	uint8_t IndexLoc = Index;
	...
	MacAddr = XEmacPs_ReadReg(InstancePtr->Config.BaseAddress,
				    ((uint32_t)XEMACPS_LADDR1L_OFFSET + ((uint32_t)IndexLoc * (uint32_t)8)));
...
long XEmacPs_SetHash(XEmacPs *InstancePtr, void *AddressPtr)
{
	...
	if (InstancePtr->IsStarted == (uint32_t)XIL_COMPONENT_IS_STARTED) {
		...
		Result = (uint32_t)((uint32_t)Temp1 ^ (uint32_t)Temp2 ^ (uint32_t)Temp3 ^ (uint32_t)Temp4 ^
				(uint32_t)Temp5 ^ (uint32_t)Temp6 ^ (uint32_t)Temp7 ^ (uint32_t)Temp8);

		if (Result >= (uint32_t)XEMACPS_MAX_HASH_BITS) {
			Status = (long)(XST_INVALID_PARAM);
		} else {

			if (Result < (uint32_t)32) {
		HashAddr = XEmacPs_ReadReg(InstancePtr->Config.BaseAddress,
				XEMACPS_HASHL_OFFSET);
				HashAddr |= (uint32_t)(0x00000001U << Result);
		XEmacPs_WriteReg(InstancePtr->Config.BaseAddress,
			XEMACPS_HASHL_OFFSET, HashAddr);
	} else {
		HashAddr = XEmacPs_ReadReg(InstancePtr->Config.BaseAddress,
				XEMACPS_HASHH_OFFSET);
				HashAddr |= (uint32_t)(0x00000001U << (uint32_t)(Result - (uint32_t)32));
		XEmacPs_WriteReg(InstancePtr->Config.BaseAddress,
...
{
	uint32_t *Aptr = (uint32_t *)(void *)AddressPtr;
 ...
 *
 *****************************************************************************/
long XEmacPs_SetTypeIdCheck(XEmacPs *InstancePtr, uint32_t Id_Check, uint8_t Index)
{
	uint8_t IndexLoc = Index;
	long Status;
	Xil_AssertNonvoid(InstancePtr != NULL);
	...
	XEmacPs_WriteReg(InstancePtr->Config.BaseAddress,
				   ((uint32_t)XEMACPS_MATCH1_OFFSET + ((uint32_t)IndexLoc * (uint32_t)4)), Id_Check);
 ...
 *
 *****************************************************************************/
long XEmacPs_SetOptions(XEmacPs *InstancePtr, uint32_t Options)
{
	...
	/* Allow broadcast address reception */
		if ((Options & XEMACPS_BROADCAST_OPTION) != 0x00000000U) {
			RegNewNetCfg &= (uint32_t)(~XEMACPS_NWCFG_BCASTDI_MASK);
	}
	...
	if (((Options & XEMACPS_JUMBO_ENABLE_OPTION) != 0x00000000U) &&
		...
		Reg |= (((((uint32_t)XEMACPS_RX_BUF_SIZE_JUMBO / (uint32_t)XEMACPS_RX_BUF_UNIT) +
			(((((uint32_t)XEMACPS_RX_BUF_SIZE_JUMBO %
			(uint32_t)XEMACPS_RX_BUF_UNIT))!=(uint32_t)0) ? 1U : 0U)) <<
			(uint32_t)(XEMACPS_DMACR_RXBUF_SHIFT)) &
			(uint32_t)(XEMACPS_DMACR_RXBUF_MASK));
		XEmacPs_WriteReg(InstancePtr->Config.BaseAddress,
 ...
 *
 *****************************************************************************/
long XEmacPs_ClearOptions(XEmacPs *InstancePtr, uint32_t Options)
{
	 ...
	 */
		if ((Options & XEMACPS_FRAME1536_OPTION) != 0x00000000U) {
			RegNewNetCfg &= (uint32_t)(~XEMACPS_NWCFG_1536RXEN_MASK);
	}

	/* Turn off VLAN packet only */
		if ((Options & XEMACPS_VLAN_OPTION) != 0x00000000U) {
			RegNewNetCfg &= (uint32_t)(~XEMACPS_NWCFG_NVLANDISC_MASK);
	}

	/* Turn off FCS stripping on receive packets */
		if ((Options & XEMACPS_FCS_STRIP_OPTION) != 0x00000000U) {
			RegNewNetCfg &= (uint32_t)(~XEMACPS_NWCFG_FCSREM_MASK);
	}

	/* Turn off length/type field checking on receive packets */
		if ((Options & XEMACPS_LENTYPE_ERR_OPTION) != 0x00000000U) {
			RegNewNetCfg &= (uint32_t)(~XEMACPS_NWCFG_LENERRDSCRD_MASK);
	}

	/* Turn off flow control */
		if ((Options & XEMACPS_FLOW_CONTROL_OPTION) != 0x00000000U) {
			RegNewNetCfg &= (uint32_t)(~XEMACPS_NWCFG_PAUSEEN_MASK);
	}

	/* Turn off promiscuous frame filtering (all frames are received) */
		if ((Options & XEMACPS_PROMISC_OPTION) != 0x00000000U) {
			RegNewNetCfg &= (uint32_t)(~XEMACPS_NWCFG_COPYALLEN_MASK);
	}
	...
	/* Disallow multicast address filtering */
		if ((Options & XEMACPS_MULTICAST_OPTION) != 0x00000000U) {
			RegNewNetCfg &= (uint32_t)(~XEMACPS_NWCFG_MCASTHASHEN_MASK);
	}

	/* Disable RX checksum offload */
		if ((Options & XEMACPS_RX_CHKSUM_ENABLE_OPTION) != 0x00000000U) {
			RegNewNetCfg &= (uint32_t)(~XEMACPS_NWCFG_RXCHKSUMEN_MASK);
	}

	/* Disable jumbo frames */
	if (((Options & XEMACPS_JUMBO_ENABLE_OPTION) != 0x00000000U) &&
		(InstancePtr->Version > 2)) {
		RegNewNetCfg &= (uint32_t)(~XEMACPS_NWCFG_JUMBO_MASK);
		Reg = XEmacPs_ReadReg(InstancePtr->Config.BaseAddress,
				      XEMACPS_DMACR_OFFSET);
		Reg &= ~XEMACPS_DMACR_RXBUF_MASK;
		Reg |= (((((uint32_t)XEMACPS_RX_BUF_SIZE / (uint32_t)XEMACPS_RX_BUF_UNIT) +
			(((((uint32_t)XEMACPS_RX_BUF_SIZE %
			(uint32_t)XEMACPS_RX_BUF_UNIT))!=(uint32_t)0) ? 1U : 0U)) <<
			(uint32_t)(XEMACPS_DMACR_RXBUF_SHIFT)) &
			(uint32_t)(XEMACPS_DMACR_RXBUF_MASK));
		XEmacPs_WriteReg(InstancePtr->Config.BaseAddress,
	...
	if (((Options & XEMACPS_SGMII_ENABLE_OPTION) != 0x00000000U) &&
		(InstancePtr->Version > 2)) {
		RegNewNetCfg &= (uint32_t)(~(XEMACPS_NWCFG_SGMIIEN_MASK |
						XEMACPS_NWCFG_PCSSEL_MASK));
	}
	...
	/* Disable TX checksum offload */
		...
		Reg = XEmacPs_ReadReg(InstancePtr->Config.BaseAddress,
					XEMACPS_DMACR_OFFSET);
			Reg &= (uint32_t)(~XEMACPS_DMACR_TCPCKSUM_MASK);
		XEmacPs_WriteReg(InstancePtr->Config.BaseAddress,
					 XEMACPS_DMACR_OFFSET, Reg);
	}
	...
	/* Disable transmitter */
		...
		Reg = XEmacPs_ReadReg(InstancePtr->Config.BaseAddress,
					XEMACPS_NWCTRL_OFFSET);
			Reg &= (uint32_t)(~XEMACPS_NWCTRL_TXEN_MASK);
		XEmacPs_WriteReg(InstancePtr->Config.BaseAddress,
				   XEMACPS_NWCTRL_OFFSET, Reg);
	}
	...
	/* Disable receiver */
		...
		Reg = XEmacPs_ReadReg(InstancePtr->Config.BaseAddress,
					XEMACPS_NWCTRL_OFFSET);
			Reg &= (uint32_t)(~XEMACPS_NWCTRL_RXEN_MASK);
		XEmacPs_WriteReg(InstancePtr->Config.BaseAddress,
				   XEMACPS_NWCTRL_OFFSET, Reg);
	}
 ...
 *
 *****************************************************************************/
uint32_t XEmacPs_GetOptions(XEmacPs *InstancePtr)
{
	Xil_AssertNonvoid(InstancePtr != NULL);
	Xil_AssertNonvoid(InstancePtr->IsReady == (uint32_t)XIL_COMPONENT_IS_READY);
 ...
 *
 *****************************************************************************/
uint16_t XEmacPs_GetOperatingSpeed(XEmacPs *InstancePtr)
{
	uint32_t Reg;
	uint16_t Status;
 ...
 *
 *****************************************************************************/
void XEmacPs_SetOperatingSpeed(XEmacPs *InstancePtr, uint16_t Speed)
{
        uint32_t Reg;
        Xil_AssertVoid(InstancePtr != NULL);
	Xil_AssertVoid(InstancePtr->IsReady == (uint32_t)XIL_COMPONENT_IS_READY);
...
*
******************************************************************************/
long XEmacPs_PhyRead(XEmacPs *InstancePtr, uint32_t PhyAddress,
		     uint32_t RegisterNum, uint16_t *PhyDataPtr)
{
	...
	/* Read data */
		*PhyDataPtr = (uint16_t)XEmacPs_ReadReg(InstancePtr->Config.BaseAddress,
...
*
******************************************************************************/
long XEmacPs_PhyWrite(XEmacPs *InstancePtr, uint32_t PhyAddress,
...
*
******************************************************************************/
void XEmacPs_DMABLengthUpdate(XEmacPs *InstancePtr, int32_t BLength)
{
	uint32_t Reg;
	uint32_t RegUpdateVal = 0;
	...
	Reg &= (uint32_t)(~XEMACPS_DMACR_BLENGTH_MASK);
	Reg |= RegUpdateVal;
