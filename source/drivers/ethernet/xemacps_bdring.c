...
#define XEMACPS_PHYS_TO_VIRT(BdPtr) \
    ((uint64_t)(BdPtr) + (RingPtr->BaseBdAddr - RingPtr->PhysBaseAddr))
...
#define XEMACPS_RING_SEEKAHEAD(RingPtr, BdPtr, NumBd)                  \
    {                                                                   \
        uint64_t Addr = (uint64_t)(void *)(BdPtr);                        \
                                                                        \
        Addr += ((RingPtr)->Separation * (NumBd));                        \
        if ((Addr > (RingPtr)->HighBdAddr) || ((uint64_t)(void *)(BdPtr) > Addr))  \
        {                                                               \
            Addr -= (RingPtr)->Length;                                  \
        }                                                               \
                                                                        \
...
#define XEMACPS_RING_SEEKBACK(RingPtr, BdPtr, NumBd)                   \
    {                                                                   \
        uint64_t Addr = (uint64_t)(void *)(BdPtr);                                  \
                                                                        \
        Addr -= ((RingPtr)->Separation * (NumBd));                        \
        if ((Addr < (RingPtr)->BaseBdAddr) || ((uint64_t)(void*)(BdPtr) < Addr))  \
        {                                                               \
            Addr += (RingPtr)->Length;                                  \
        }                                                               \
                                                                        \
...
static void XEmacPs_BdSetRxWrap(uint64_t BdPtr);
static void XEmacPs_BdSetTxWrap(uint64_t BdPtr);
			  ...
			  uint64_t VirtAddr, uint32_t Alignment, uint32_t BdCount)
{
	...
	if (Alignment < (uint32_t)XEMACPS_DMABD_MINIMUM_ALIGNMENT) {
	...
	if (((PhysAddr % Alignment)!=(uint32_t)0) || ((VirtAddrLoc % Alignment)!=(uint32_t)0)) {
		return (long)(XST_INVALID_PARAM);
	}
	...
	RingPtr->Separation = ((uint32_t)sizeof(XEmacPs_Bd));

	/* Must make sure the ring doesn't span address 0x00000000. If it does,
	 * then the next/prev BD traversal macros will fail.
	 */
	if (VirtAddrLoc > ((VirtAddrLoc + (RingPtr->Separation * BdCount)) - (uint32_t)1)) {
	...
	RingPtr->RunState = (uint32_t)(XST_DMA_SG_IS_STOPPED);
	...
	RingPtr->AllCnt = (uint32_t)BdCount;
	RingPtr->FreeCnt = (uint32_t)BdCount;
			 ...
			 uint8_t Direction)
{
	...
	if (RingPtr->RunState == (uint32_t)XST_DMA_SG_IS_STARTED) {
	...
	if ((Direction != (uint8_t)XEMACPS_SEND) && (Direction != (uint8_t)XEMACPS_RECV)) {
		return (long)(XST_INVALID_PARAM);
	}
				 ...
				 XEmacPs_Bd ** BdSetPtr)
{
	...
	uint32_t BdStr = 0U;
	uint32_t BdCount;
	uint32_t BdPartialCount;
	uint32_t Sop = 0U;
	uint32_t Status;
	uint32_t BdLimitLoc = BdLimit;
...
long XEmacPs_BdRingCheck(XEmacPs_BdRing * RingPtr, uint8_t Direction)
{
	...
	if (RingPtr->RunState != (uint32_t)XST_DMA_SG_IS_STOPPED) {
		return (long)(XST_DMA_SG_LIST_ERROR);
	}

	/* Verify internal pointers point to correct memory space */
	AddrV = (uint64_t) RingPtr->FreeHead;
	if ((AddrV < RingPtr->BaseBdAddr) || (AddrV > RingPtr->HighBdAddr)) {
		return (long)(XST_DMA_SG_LIST_ERROR);
	}

	AddrV = (uint64_t) RingPtr->PreHead;
	if ((AddrV < RingPtr->BaseBdAddr) || (AddrV > RingPtr->HighBdAddr)) {
		return (long)(XST_DMA_SG_LIST_ERROR);
	}

	AddrV = (uint64_t) RingPtr->HwHead;
	if ((AddrV < RingPtr->BaseBdAddr) || (AddrV > RingPtr->HighBdAddr)) {
		return (long)(XST_DMA_SG_LIST_ERROR);
	}

	AddrV = (uint64_t) RingPtr->HwTail;
	if ((AddrV < RingPtr->BaseBdAddr) || (AddrV > RingPtr->HighBdAddr)) {
		return (long)(XST_DMA_SG_LIST_ERROR);
	}

	AddrV = (uint64_t) RingPtr->PostHead;
	if ((AddrV < RingPtr->BaseBdAddr) || (AddrV > RingPtr->HighBdAddr)) {
		return (long)(XST_DMA_SG_LIST_ERROR);
	}
...
static void XEmacPs_BdSetRxWrap(uint64_t BdPtr)
{
    uint32_t DataValueRx;
	uint32_t *TempPtr;

	BdPtr += (uint32_t)(XEMACPS_BD_ADDR_OFFSET);
	TempPtr = (uint32_t *)BdPtr;
	if(TempPtr != NULL) {
		DataValueRx = *TempPtr;
...
static void XEmacPs_BdSetTxWrap(uint64_t BdPtr)
{
    uint32_t DataValueTx;
	uint32_t *TempPtr;

	BdPtr += (uint32_t)(XEMACPS_BD_STAT_OFFSET);
	TempPtr = (uint32_t *)BdPtr;
	if(TempPtr != NULL) {
		DataValueTx = *TempPtr;
