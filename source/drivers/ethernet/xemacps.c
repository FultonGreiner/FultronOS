...
void XEmacPs_Start(XEmacPs *InstancePtr)
{
	...
	if ((InstancePtr->Options & (uint32_t)XEMACPS_TRANSMITTER_ENABLE_OPTION)!=0x00000000U) {
	...
	XEMACPS_IXR_RX_ERR_MASK | (uint32_t)XEMACPS_IXR_FRAMERX_MASK |
	(uint32_t)XEMACPS_IXR_TXCOMPL_MASK));
...
void XEmacPs_Reset(XEmacPs *InstancePtr)
{
	...
	XEmacPs_WriteReg(InstancePtr->Config.BaseAddress,
			...
			(uint32_t)(~XEMACPS_NWCTRL_LOOPEN_MASK));

	Reg = XEmacPs_ReadReg(InstancePtr->Config.BaseAddress,
			XEMACPS_NWCFG_OFFSET);
	Reg &= XEMACPS_NWCFG_MDCCLKDIV_MASK;

	Reg = Reg | (uint32_t)XEMACPS_NWCFG_100_MASK |
			(uint32_t)XEMACPS_NWCFG_FDEN_MASK |
			(uint32_t)XEMACPS_NWCFG_UCASTHASHEN_MASK;
	...
	XEmacPs_WriteReg(InstancePtr->Config.BaseAddress,
			...
			(((((uint32_t)XEMACPS_RX_BUF_SIZE / (uint32_t)XEMACPS_RX_BUF_UNIT) +
				...
				(uint32_t)XEMACPS_DMACR_RXSIZE_MASK |
				(uint32_t)XEMACPS_DMACR_TXSIZE_MASK);
	...
	if (InstancePtr->Version > 2) {
		XEmacPs_WriteReg(InstancePtr->Config.BaseAddress, XEMACPS_DMACR_OFFSET,
			...
			(uint32_t)XEMACPS_DMACR_ADDR_WIDTH_64 |
#endif
			(uint32_t)XEMACPS_DMACR_INCR16_AHB_BURST));
	}

	XEmacPs_WriteReg(InstancePtr->Config.BaseAddress,
			   XEMACPS_TXSR_OFFSET, XEMACPS_SR_ALL_MASK);

	XEmacPs_SetQueuePtr(InstancePtr, 0, 0x00U, (uint16_t)XEMACPS_SEND);
	if (InstancePtr->Version > 2)
		XEmacPs_SetQueuePtr(InstancePtr, 0, 0x01U, (uint16_t)XEMACPS_SEND);
	XEmacPs_SetQueuePtr(InstancePtr, 0, 0x00U, (uint16_t)XEMACPS_RECV);
	...
	/* clear all counters */
	for (i = 0U; i < (uint8_t)((XEMACPS_LAST_OFFSET - XEMACPS_OCTTXL_OFFSET) / 4U);
	     i++) {
		(void)XEmacPs_ReadReg(InstancePtr->Config.BaseAddress,
                                   XEMACPS_OCTTXL_OFFSET + (uint32_t)(((uint32_t)i) * ((uint32_t)4)));
	}
			 ...
			 uint16_t Direction)
{
	...
	if (Direction == XEMACPS_SEND) {
		...
		XEmacPs_WriteReg(InstancePtr->Config.BaseAddress,
				...
				(uint32_t)((QPtr & ULONG64_HI_MASK) >> 32U));
	} else {
